...
namespace TrackerApi.Controllers
{
    ...
    public class HistoriesController : ApiController
    {
        ...
        public IHttpActionResult GetHistories()
        {
            ...
            return Ok(_context.Historis.Where(h => h.viewFlag == true).Select(h => h).Include(his => his.Child));
        }
        //GET /api/Histories/1
        public IHttpActionResult GetHistory(int id)
        {
            var history = _context.Historis.SingleOrDefault(h => h.Id == id);
            if (history==null)
            {
                return NotFound();
            }
        ...
        public IHttpActionResult CreateHistory([FromBody] LocationHistory history)
        {
            ...
            return Created(new Uri(Request.RequestUri + "/" + history.Id), history);
        }
        [HttpPut]
        ...
        {
            LocationHistory history = _context.Historis.SingleOrDefault(h => h.Id == id);
            if (history==null)
            {
                return NotFound();
            }
